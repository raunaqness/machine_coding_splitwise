...
class Bookkeeper:
    ...
    def view_balance(self, user_name):
        ...
        if user is None:
            raise Exception("User with Name : {} does not exist.".format(user_name))
    ...
    def add_expense(self, added_by, paid_by, expense_type, total_amount, users_data):
        ...
        if expense_type == "equal":
            ...
            for name in users_data:
                ...
                if user is None:
                    raise Exception("User with Name : {} does not exist.".format(name))
